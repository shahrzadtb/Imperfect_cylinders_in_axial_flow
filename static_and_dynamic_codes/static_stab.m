    ...
    for s = 3 : N
     ...
     for s = 1 : N
            for r = 1 : N
                    ...
                    Cmat(s,r) = 2 * xi * u * ( cos( theta0 ) ) * sqrt ( betta ) * B(s,r) ...
                        + 0.5 * ( kronecker_delta(s,r) ) * ( epsilon_cn * u * ( 1 / ( cos ( theta0 ) ) ) * sqrt ( betta ) ...
                        + epsilon_c * ( 1 / ( ( cos ( theta0 ) ) ^ 2 ) ) * sqrt ( betta ) ) ...
                        - ( f * xi * u * ( cos ( theta0 ) ) * sqrt ( betta ) ...
                        - 0.5 * khi_e_bar * epsilon_cn * u * ( 1 / ( cos ( theta0 ) ) ) * sqrt ( betta )...
                        - 0.5 * epsilon_c * ( 1 / ( ( cos ( theta0 ) ) ^ 2 ) ) * khi_e_bar * sqrt ( betta ) ) * phi_1(1,r) * phi_1(1,s)...
                        + xi * khi_e * f * u * ( cos ( theta0 ) ) * sqrt ( betta ) * phi_1_prime(1,r) * phi_1(1,s);
                    
                    
                    
                    Kmat(s,r) = ( kronecker_delta ( s,r ) ) * ( lam ( r ) ) ^ 4 + xi * ( u ^ 2 ) * ( ( cos( theta0 ) ) ^ 2 ) * C(s,r)...
                        - ( 0.5 * epsilon_ct * ( u ^ 2 ) * ( 1 + h ) + gamma * ( cos ( theta0 ) ) ) *( C(s,r) - D(s,r) )...
                        - ( 0.5 * cb * ( u ^ 2 ) + 0.5 * epsilon_ct * ( u ^ 2 ) * khi_e_bar + gamma * khi_e * ( cos ( theta0 ) ) + 0.5 * epsilon_ct * ( u ^ 2 ) * h * khi_e ) * C(s,r)...
                        + ( 0.5 * epsilon_cn * ( u ^ 2 ) + gamma * ( cos ( theta0 ) ) + 0.5 * epsilon_ct * u ^ 2 * h ) * B (s,r) ...
                        - ( - 0.5 * khi_e_bar * epsilon_cn * ( u ^ 2 ) ...
                        + f * xi * ( u ^ 2 ) * ( cos ( 2 * ( theta0 ) ) ) ...
                        - gamma * khi_e * ( cos ( theta0 ) ) ...
                        - 0.5 * epsilon_ct * ( u ^ 2 ) * h * khi_e ) * phi_1_prime(1,r) * phi_1(1,s)...
                        - ( 0.5 * epsilon_ct * ( u ^ 2 ) * ( 1 + khi_e_bar ) + 0.5 * cb * ( u ^ 2 ) ...
                        + gamma * khi_e * ( cos ( theta0 ) ) + gamma * ( cos ( theta0 ) ) ...
